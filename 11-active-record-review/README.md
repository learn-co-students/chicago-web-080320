...
For every one magazine, how many subscriptions?
For every one subscription, how many magazines?
...
TO TEST: 
```rb
# Subscription.first - get an instance from the database
# call instance methods on the instance
Subscription.first.reader
Subscription.first.magazine
```
...
TO TEST: 
```rb
Magazine.first.subscriptions
Magazine.first.readers
```
#### Reader
